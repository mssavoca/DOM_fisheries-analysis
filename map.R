...
NE=c("maine","new hampshire","maryland","massachusetts","connecticut", "rhode island","new jersey", "new york", "west virginia","delaware","west virginia","virginia")
...
p=p+  geom_scatterpie(aes(x=x,y=y,r = 4,),data=bycatch,cols=colnames(bycatch)[2:10],color=NA)+coord_fixed() +
  ...
  annotate("text",x=-117.5,y=14,label="Alaska",size=2,color="#555555")+annotate("text",x=-68,y=31,label="New England",size=2,color="#555555")+
...
png("test.png",width=5, height=5, units="in", res=400)
par(ps=10)
par(mar=c(1,1,1,1))
par(cex=1)
p
